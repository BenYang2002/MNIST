classdef MNISTLayer < handle
    ...
    methods
        function this = MNISTLayer(wMat1,wMat2, bVect1, bVect2 ...
          ,learning_rate,transfer,acceptance, training, trainingTimes, ... 
          MNIST, wMat3, bVect3, wMat4, bVect4)
            ...
            if (exist('wMat3', 'var'))
                this.wMat3 = wMat3;
                this.bVect3 = bVect3;
                this.layers{3} = [this.wMat3,this.bVect3];
            end
            if (exist('wMat4', 'var'))
                this.wMat4 = wMat4;
                this.bVect4 = bVect4;
                this.layers{4} = [this.wMat4,this.bVect4];  
            end
        ...
        function train(this, inputMatrix, expectedM)
            ...
            epoch = 1;
            correct = false;
            figure("Name","Perfomance Index Over i Iterations");
            while (~correct && iter <= this.trainingTimes)
                correct = true;
                disp("iter: " + epoch);
                iter = 1;
                for i = 1 : size(inputMatrix,2)
                    ...
                    plot(zeros(length(this.prediction), 1) + epoch, pIndex, color);
                    if mod(iter, 30) == 0
                        drawnow();
                    end
                    hold on
            ...
            title('Perfomance Index Over i Iterations');
